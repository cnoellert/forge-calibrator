...
`_JPEG_PRESET` constant and `_NoHooks` class still present but unused after the Wiretap migration. `flame/apply_solve.py` and `flame/solve_and_update.py` were removed in quick-260505-mrv (2026-05-05) along with `flame/action_export.py`. `matchbox/` stays as a repo artifact of the shelved direction (memory/matchbox_direction_shelved.md). Also: `_apply_camera` still inlines the action-camera discovery loop that's now available as `_find_action_cameras()` at module level — fold them together next time that path is touched (noted in the helper's docstring).
