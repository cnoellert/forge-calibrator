...
Last activity: 2026-05-06 -- README + docs sweep (quick 260506-93a): deleted docs/seamless-bridge.md (283 LOC) + tools/smoke-test/seamless-bridge-smoke.sh (companion to A2 strip — both described the deleted feature); rewrote README.md 98→60 LOC to match calibrator's pristine VP-solve identity; STRUCTURE.md tree refreshed. Three known follow-ups (orphan FPS-label helper in camera_match_hook.py, CLAUDE.md mirror sections, PASSOFF.md historical mention) all flagged for separate sweeps. 191 tests still passing.
...
| 260506-93a | README + docs sweep — companion to A2 strip; deleted docs/seamless-bridge.md (283 LOC) + tools/smoke-test/seamless-bridge-smoke.sh, rewrote README.md 98→60 LOC to match calibrator's pristine VP-solve identity, refreshed STRUCTURE.md tree. 3 commits, -571 LOC, 191 tests still passing. Three known follow-ups (orphan FPS-label helper, CLAUDE.md mirror sections, PASSOFF.md historical mention) flagged for separate sweeps. | 2026-05-06 | 0011c60 | [260506-93a-readme-docs-sweep-rewrite-readme-md-dele](./quick/260506-93a-readme-docs-sweep-rewrite-readme-md-dele/) |
| 260512-sudo-install-env | Installer sudo-context fix — default forge env now resolves from the invoking user's home instead of `/var/root`, and conda lookup checks common miniconda/anaconda paths when sudo strips PATH; verified with normal and simulated sudo dry-runs | 2026-05-12 | this commit | [260512-sudo-install-env](./quick/260512-sudo-install-env/) |
| 260512-calibrator-scene-scale | Camera Calibrator scene scale — added 0.001x/0.01x/0.1x/1.0x scale selector with 0.01x default; adapter uniformly scales camera/helper world positions while preserving origin projection, FOV, focal, and rotation; +2 projection/scale tests | 2026-05-12 | this commit | [260512-calibrator-scene-scale](./quick/260512-calibrator-scene-scale/) |
| 260512-third-axis-handedness | Camera Calibrator third-axis overlay fix — out-of-plane axis now uses solver handedness (`VP1 axis × VP2 axis`), so `VP1=-Z` + `VP2=-Y` implies `-X` instead of forcing `+X`; +2 helper tests | 2026-05-12 | this commit | [260512-third-axis-handedness](./quick/260512-third-axis-handedness/) |
