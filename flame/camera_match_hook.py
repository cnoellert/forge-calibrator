...
def _scope_action_camera(selection):
    ...
    inside an Action schematic. Originally added in Plan 04.4-03 as the
    `isVisible` predicate for the (now-removed) get_action_custom_ui_actions
    Blender export menu; SURVIVES post quick-260505-tb3 because the Apply
    Camera picker flow inside _open_camera_match still uses it.

    CRITICAL (RESEARCH §Pitfall 1): in Action-context callbacks,
    item.type is a plain Python str — NOT a PyAttribute. Compare with
    `item.type == "Camera"` directly. Calling .get_value() raises
    AttributeError, the try/except below swallows it, and the predicate
    silently returns False. The Wave 0 test
    ...
    position/rotation/fov/focal API. Allowlist both type strings here so
    the predicate matches on either variant.
    """
    import flame
    for item in selection:
        try:
...
def get_batch_custom_ui_actions():
    ...
    The Blender export AND import entries were removed in
    quick-260505-tb3 (Phase A2 strip — bridge-free, one-way only
    architecture decision locked 2026-05-05 across calibrator +
    forge-blender sessions). forge-blender Phase 6 (v1.1) will absorb
    the export half via cherry-pick from this repo's git history.
    Calibrator is now a single-purpose VP-solve tool.
    """
    return [
        {
            "name": "FORGE",
